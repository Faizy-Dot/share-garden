...
class ShareService {
    // Share SGTip with native share dialog
    ...
    async shareToPlatform(platform, sgTipData, sgTipId) {
        ...
        switch (platform) {
            case 'whatsapp':
                return this.shareToWhatsApp(shareText, shareUrl);
            case 'facebook':
            ...
            case 'copy':
                return this.copyToClipboard(shareText, shareUrl);
            default:
    ...
    // Share to WhatsApp
    async shareToWhatsApp(shareText, shareUrl) {
    ...
    async shareToInstagram(shareText) {
        try {
            ...
            Alert.alert(
                'Copied to Clipboard',
                'The SGTip content has been copied to your clipboard. You can now paste it in Instagram Stories or posts.',
                [
                    {
                        text: 'Open Instagram',
                        onPress: () => {
                            ...
                            Linking.canOpenURL(instagramUrl).then(supported => {
                                if (supported) {
                                    ...
                                    Alert.alert('Instagram not installed', 'Please install Instagram to continue.');
                                }
                            });
                        }
                    },
    ...
    // Copy to clipboard
    async copyToClipboard(shareText, shareUrl) {
        try {
            ...
            Alert.alert(
                'Copied to Clipboard',
                'SGTip content has been copied to your clipboard.',
                [{ text: 'OK' }]
            );
    ...
    showShareOptions(sgTipData, sgTipId, onShareComplete) {
        Alert.alert(
            'Share SGTip',
            'Choose how you want to share this SGTip:',
            [
                {
                    text: 'Native Share',
                    onPress: () => this.shareSGTip(sgTipData, sgTipId).then(onShareComplete)
                },
                {
                    text: 'WhatsApp',
                    onPress: () => this.shareToPlatform('whatsapp', sgTipData, sgTipId).then(onShareComplete)
                },
                {
                    text: 'Facebook',
                    onPress: () => this.shareToPlatform('facebook', sgTipData, sgTipId).then(onShareComplete)
                },
                {
                    text: 'Twitter',
                    onPress: () => this.shareToPlatform('twitter', sgTipData, sgTipId).then(onShareComplete)
                },
                {
                    text: 'Instagram',
                    onPress: () => this.shareToPlatform('instagram', sgTipData, sgTipId).then(onShareComplete)
                },
                {
                    text: 'Copy Link',
                    onPress: () => this.shareToPlatform('copy', sgTipData, sgTipId).then(onShareComplete)
                },
                {
                    text: 'Cancel',
                    style: 'cancel'
